...
describe('VxgBasicLedPlugin', () => {
  ...
  test('ready-list', async () => {
    ...
    expect(store.getState().main.view.foo)
      .equal({ mode: 'list', status: 'init', ready: true })

    await seneca.post('aim:app,on:BasicLed,ready:list,view:foo',{setReady:()=>null})
    expect(store.getState().main.view.foo)
      .equal({ mode: 'list', status: 'list-item', ready: true })
    await seneca.close()
  })
  ...
  test('ready-edit', async () => {
    ...
    await seneca.post('aim:app,on:BasicLed,ready:edit,view:foo',{setReady:()=>null})
    expect(store.getState().main.view.foo)
      .equal({ mode: 'edit', status: 'edit-item', ready: true })
    await seneca.close()
  })
