...
function BasicSide (props: any) {
  ...
  const viewdefs = Object.entries(viewmap).map(
    (entry: any) => ((entry[1].name = entry[0]), entry[1])
  )

  const sectiondefs = Object.entries(part.section || []).map(
    (entry: any) => ((entry[1].name = entry[0]), entry[1])
  )

  const viewPath: any = location.pathname.split('/')[2]

  const [showViewsData, setShowViewsData] = useState(
    sectiondefs.map((section: any, sectionNumber: number) => {
      return (
        viewPath == section.name || (section.view && viewPath in section.view)
      )
    })
  )
  const [toogleSelections, setToogleSelections] = useState({
    [viewPath]: true
  } as any)
  ...
  function sortViews (viewdefs: any, viewOrder: any) {
    const orderedViews = Object.keys(viewOrder).map(
      viewName =>
  ...
  return (
    <BasicDrawer variant='permanent' drawerwidth={drawerwidth} open={open}>
      <BasicDrawerHeader>
        <img src={part.logo.img} style={{ width: '11rem' }} />
      ...
      {sectiondefs.map((section: any, sectionNumber: number) => {
        if (
          viewPath == section.name ||
          (section.view && viewPath in section.view)
        ) {
          if ('navmenu' === section.kind) {
            return (
              <DefaultNavMenu
                key={section.name}
                viewOrder={section.view}
                viewdefs={viewdefs}
              />
            )
          }
          ...
          return <Cmp key={section.name} ctx={ctx} spec={spec} />
        }
      })}
    </BasicDrawer>
  )
